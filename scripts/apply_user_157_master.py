...
# The exact 157 items provided by the user, paired with 100% accurate psychological evaluation answers
user_statements_and_answers = [
...
print(f"Total user items: {len(user_statements_and_answers)}")
assert len(user_statements_and_answers) == 157

# Generate HTML SOAL (Exact user list)
col1 = user_statements_and_answers[:79]
col2 = user_statements_and_answers[79:]
...
<body>
  ...
  <div class="header">
    ...
    <div class="badge">SESI 3 (157 BUTIR LENGKAP)</div>
  </div>

  <div class="instructions">
    <div><strong>Petunjuk:</strong> Bacalah setiap nomor pernyataan, lalu tentukan respon Anda pada Lembar Jawaban:</div>
...
html_soal += """
  ...
  <div class="footer">
    ...
    <span>Total 157 Butir Pertanyaan Lengkap</span>
  </div>
...
# Generate HTML KUNCI JAWABAN (Exact 1..157 matching user's list)
chunks = [
    user_statements_and_answers[0:32],
    user_statements_and_answers[32:64],
    user_statements_and_answers[64:96],
    user_statements_and_answers[96:127],
    user_statements_and_answers[127:157]
]
...
<body>
  ...
  <div class="confidential-box">
    <strong>KUNCI JAWABAN AKURAT (157 BUTIR):</strong> Kunci jawaban ini disinkronkan 100% dengan Dokumen Soal Sesi 3 nomor 1 s/d 157 tanpa ada butir baru, berdasarkan respon optimal kandidat profesional.
  </div>
...
print("Generated HTML files for Soal & Kunci Jawaban (157 exact user items)")

# Update defaultConfig.ts
lines = []
for num, _, ans in user_statements_and_answers:
...
storage_code = storage_code.replace("ati_form_settings_v3", "ati_form_settings_v4")
storage_code = storage_code.replace("ati_answer_keys_sesi_3_v3", "ati_answer_keys_sesi_3_v4")

with open("c:/form ATI/src/services/storage.ts", "w", encoding="utf-8") as f:
    f.write(storage_code)

print("Updated storage keys to v4")
